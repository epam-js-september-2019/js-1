...
const stringExpansion = (code) => {
  ...
  while (i <= chars.length) {
    let number = parseInt(chars[i], 10);
    if (!number) {
      newChars.push(chars[i]);
      i++;
      while (isNaN(parseInt(chars[i], 10)) && i <= chars.length) {
    ...
    while (parseInt(chars[i + 1], 10) === number && i <= chars.length) {
      i++;
    }
    i++;
    let char = chars[i];
    if (parseInt(chars[i], 10)) {
      throw new Error(`wrong format of string: number ${parseInt(chars[i], 10)} after number ${number} - it should be char after number! (or the same number)`);
    }
    while (chars[i + 1] === char && i <= chars.length) {
      i++;
    }
...
(()=>{
  ...
  console.log(`stringExpansion('3D2a5d2f') result:`);
  console.log(stringExpansion('3D2a5d2f'));
  console.log(`stringExpansion('abcdef') result:`);
  console.log(stringExpansion('abcdef'));
  console.log(`stringExpansion('33aaab44cdddef') result:`);
  console.log(stringExpansion('33aaab44cdddef'));
